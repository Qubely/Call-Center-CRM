...
use App\Http\Requests\Admin\Client\ClientList\Crud\ValidateUpdateClient;
...
class  ClientCrudRepository extends BaseRepository implements IClientCrudRepository {
    ...
    {
       return $this->getPageDefault(model: $this->Client, id: $id);
    }
    ...
    public function store($request) : JsonResponse
    {
        try {
            Client::create([
                ...$request->all(),
                //'serial' => $this->facSrWc($this->Client)
            ]);
            $response['extraData'] = ['inflate' => pxLang($request->lang,'','common.action_success') ];
            return $this->response(['type' => 'success', 'data' => $response]);
        } catch (\Exception $e) {
            $this->saveError($this->getSystemError(['name' => 'Client_store_error']), $e);
    ...
    public function update($request,$id) : JsonResponse
    {
        ...
        if($row->isDirty()){
            $validator = Validator::make($request->all(), (new ValidateUpdateClient())->rules($request,$row));
            if ($validator->fails()) {
            ...
            try {
                ...
                $this->saveError($this->getSystemError(['name'=>'Client_update_error']), $e);
    ...
    {
        $i = Client::whereIn('id',$request->ids)->select(['id','name'])->get();;
        $dirty = [];
        if (count($i) > 0) {
    ...
    {
        $errors = [];
        $i = Client::whereIn('id',$request->ids)->select(['id'])->get();
        if (count($i) > 0) {
