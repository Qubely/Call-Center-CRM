...
use App\Http\Requests\Admin\Campaign\List\Crud\ValidateUpdateCampaign;
...
use Auth;
use DB;
class  CampaignCrudRepository extends BaseRepository implements ICampaignCrudRepository {

    use BaseTrait;
    public function __construct() {
        $this->LoadModels(['Campaign']);
    }
    ...
    {
       return $this->getPageDefault(model: $this->Campaign, id: $id);
    }
    ...
    public function list($request) : JsonResponse
    {
        ...
        $this->saveTractAction(
            $this->getTrackData(
                title: 'Campaign was viewed by '.$request?->auth?->name.' at '.Carbon::now()->format('d M Y H:i:s A'),
                request: $request,
                onlyTitle: true
            )
        );
    ...
    {   
        ...
        try {
            Campaign::create([
                ...$request->all(),
                //'serial' => $this->facSrWc($this->Campaign)
            ]);
            $response['extraData'] = ['inflate' => pxLang($request->lang,'','common.action_success') ];
            $this->saveTractAction($this->getTrackData(title: "Campaign was created by ".$request?->auth?->name,request: $request));
            DB::commit();
            return $this->response(['type' => 'success', 'data' => $response]);
        } catch (\Exception $e) {
            DB::rollback();
            $this->saveError($this->getSystemError(['name' => 'Campaign_store_error']), $e);
    ...
    public function update($request,$id) : JsonResponse
    {
        ...
        if($row->isDirty()){
            $validator = Validator::make($request->all(), (new ValidateUpdateCampaign())->rules($request,$row));
            if ($validator->fails()) {
            ...
            try {
                ...
                $this->saveError($this->getSystemError(['name'=>'Campaign_update_error']), $e);
    ...
    {
        $i = Campaign::whereIn('id',$request->ids)->select(['id','name'])->get();;
        $dirty = [];
        if (count($i) > 0) {
            ...
            if (count($dirty) > 0) {
                ...
                try {
                    ...
                    $this->saveTractAction($this->getTrackData(title: "Campaign list was updated by ".$request?->auth?->name, request: $request));
                    DB::commit();
                    return $this->response(['type' => 'success','data' => $data]);
                } catch (\Exception $e) {
                    DB::rollback();
                    $this->saveError($this->getSystemError(['name' => 'Campaign_bulk_update_error']), $e);
                    return $this->response(['type' => 'wrong', 'lang' => 'server_wrong']);
                }
            } else {
                return $this->response(['type' => 'noUpdate', 'title' =>  '<span class="text-success"> '.pxLang($request->lang,'','common.no_change').'  </span>']);
    ...
    {
        $errors = [];
        $i = Campaign::whereIn('id',$request->ids)->select(['id'])->get();
        if (count($i) > 0) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: "Campaign list was deleted by ".$request?->auth?->name, request: $request));
                DB::commit();
                return $this->response(['type' => 'success',"data"=>$data]);
            } catch (\Exception $e) {
                DB::rollback();
