...
use App\Http\Requests\Admin\Agent\Application\Crud\ValidateUpdateAgentApplication;
...
class  AgentApplicationCrudRepository extends BaseRepository implements IAgentApplicationCrudRepository {

    use BaseTrait;
    public function __construct() {
        $this->LoadModels(['AgentApplication']);
        $this->sizes =  [
    ...
    {
       return $this->getPageDefault(model: $this->AgentApplication, id: $id);
    }
    ...
    {
        $model = AgentApplication::query();
        $this->saveTractAction(
            $this->getTrackData(
                title: 'AgentApplication was viewed by '.$request?->auth?->name.' at '.Carbon::now()->format('d M Y H:i:s A'),
                request: $request,
                onlyTitle: true
            )
        );
        ...
        ->addColumn('image', function($item) {
         ...
         ->editColumn('status', function($item) {
            return  ($item?->status == 'Active') ? "<span class='badge bg-success'> Active </span>" : "<span class='badge bg-danger'> Pending </span>";
        })
        ->escapeColumns([])
        ->make(true);
    }
    ...
    public function store($request) : JsonResponse
    {
        ...
        try {
            ...
            $this->saveTractAction($this->getTrackData(title: "AgentApplication was created by ".$request?->auth?->name,request: $request));
            DB::commit();
            return $this->response(['type' => 'success', 'data' => $response]);
        } catch (\Exception $e) {
            DB::rollback();
            $this->saveError($this->getSystemError(['name' => 'AgentApplication_store_error']), $e);
    ...
    public function update($request,$id) : JsonResponse
    {
        ...
        if($row->isDirty()){
            $validator = Validator::make($request->all(), (new ValidateUpdateAgentApplication())->rules($request,$row));
            if ($validator->fails()) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: " AgentApplication ".$row?->name.' was updated by '.$request?->auth?->name,request: $request, row: $rowRef, type: 'to'));
                DB::commit();
                return $this->response(['type' => 'success','data' => $data]);
            } catch (\Exception $e) {
                $this->saveError($this->getSystemError(['name'=>'AgentApplication_update_error']), $e);
    ...
    {
        $i = AgentApplication::whereIn('id',$request->ids)->select(['id','name'])->get();;
        $dirty = [];
        if (count($i) > 0) {
            ...
            if (count($dirty) > 0) {
                ...
                try {
                    ...
                    $this->saveTractAction($this->getTrackData(title: "AgentApplication list was updated by ".$request?->auth?->name, request: $request));
                    DB::commit();
                    return $this->response(['type' => 'success','data' => $data]);
                } catch (\Exception $e) {
                    DB::rollback();
                    $this->saveError($this->getSystemError(['name' => 'AgentApplication_bulk_update_error']), $e);
                    return $this->response(['type' => 'wrong', 'lang' => 'server_wrong']);
                }
            } else {
                return $this->response(['type' => 'noUpdate', 'title' =>  '<span class="text-success"> '.pxLang($request->lang,'','common.no_change').'  </span>']);
    ...
    {
        $errors = [];
        $i = AgentApplication::whereIn('id',$request->ids)->select(['id'])->get();
        if (count($i) > 0) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: "AgentApplication list was deleted by ".$request?->auth?->name, request: $request));
                DB::commit();
                return $this->response(['type' => 'success',"data"=>$data]);
            } catch (\Exception $e) {
                DB::rollback();
