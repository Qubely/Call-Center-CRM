...
namespace App\Repositories\Admin\DataLibrary\CampaignType\Crud;

use App\Http\Requests\Admin\DataLibrary\CampaignType\Crud\ValidateUpdateLibCampaignType;
use App\Models\LibCampaignType;
...
use Auth;
use DB;
class  LibCampaignTypeCrudRepository extends BaseRepository implements ILibCampaignTypeCrudRepository {

    use BaseTrait;
    public function __construct() {
        $this->LoadModels(['LibCampaignType']);
    }
    ...
    {
       return $this->getPageDefault(model: $this->LibCampaignType, id: $id);
    }
    ...
    {
        $model = LibCampaignType::query();
        $this->saveTractAction(
            $this->getTrackData(
                title: 'LibCampaignType was viewed by '.$request?->auth?->name.' at '.Carbon::now()->format('d M Y H:i:s A'),
                request: $request,
                onlyTitle: true
            )
        );
    ...
    public function store($request) : JsonResponse
    {
        ...
        try {
            LibCampaignType::create([
                ...$request->all(),
                //'serial' => $this->facSrWc($this->LibCampaignType)
            ]);
            $response['extraData'] = ['inflate' => pxLang($request->lang,'','common.action_success') ];
            $this->saveTractAction($this->getTrackData(title: "LibCampaignType was created by ".$request?->auth?->name,request: $request));
            DB::commit();
            return $this->response(['type' => 'success', 'data' => $response]);
        } catch (\Exception $e) {
            DB::rollback();
            $this->saveError($this->getSystemError(['name' => 'LibCampaignType_store_error']), $e);
    ...
    public function update($request,$id) : JsonResponse
    {
        ...
        if($row->isDirty()){
            $validator = Validator::make($request->all(), (new ValidateUpdateLibCampaignType())->rules($request,$row));
            if ($validator->fails()) {
            ...
            try {
                ...
                $this->saveError($this->getSystemError(['name'=>'LibCampaignType_update_error']), $e);
    ...
    {
        $i = LibCampaignType::whereIn('id',$request->ids)->select(['id','name'])->get();;
        $dirty = [];
        if (count($i) > 0) {
            ...
            if (count($dirty) > 0) {
                ...
                try {
                    ...
                    $this->saveTractAction($this->getTrackData(title: "LibCampaignType list was updated by ".$request?->auth?->name, request: $request));
                    DB::commit();
                    return $this->response(['type' => 'success','data' => $data]);
                } catch (\Exception $e) {
                    DB::rollback();
                    $this->saveError($this->getSystemError(['name' => 'LibCampaignType_bulk_update_error']), $e);
                    return $this->response(['type' => 'wrong', 'lang' => 'server_wrong']);
                }
            } else {
                return $this->response(['type' => 'noUpdate', 'title' =>  '<span class="text-success"> '.pxLang($request->lang,'','common.no_change').'  </span>']);
    ...
    {
        $errors = [];
        $i = LibCampaignType::whereIn('id',$request->ids)->select(['id'])->get();
        if (count($i) > 0) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: "LibCampaignType list was deleted by ".$request?->auth?->name, request: $request));
                DB::commit();
                return $this->response(['type' => 'success',"data"=>$data]);
            } catch (\Exception $e) {
                DB::rollback();
