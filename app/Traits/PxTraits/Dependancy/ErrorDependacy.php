...
trait ErrorDependacy
{
     ...
     public function getTrackData($title,$request,$onlyTitle=false,$row=null,$type='from')
    {
        ...
        if($onlyTitle) {
            ...
            if($type == 'from') {
                $fromData = implode(', ', array_map(fn($k,$v)=> $k.': '.$v, array_keys($rData),$rData));
            }
            if($type == 'to') {
                $fromData = implode(', ', array_map(fn($k,$v)=>$k.' : '.$v, array_keys($row),$row));;
                $toData =  implode(', ', array_map(fn($k,$v)=>$k.' : '.$v, array_keys($rData),$rData));
            }
        }
        return [
            "title"      => $title,
