<div class="bg-info pl-2 page-fragment-bar">
    <span class="text-light"> <a href="{{url('admin/campaign/list')}}"><span class="badge badge-info cursor-pointer"> <i class='fa-solid fa-arrow-left fs-16'></i></span></a> <span class="pt-1"> {{pxLang($data['lang'],'update')}}   </span> </span>
</div>
<div class="mt-4 p-3">
    @can('campaign_crud_edit')
        <form id="frmUpdateCampaign" autocomplete="off">
            @method('PATCH')
                        ...
                        <div class="col-md-4">
                            <div class="card p-2 shadow-card card-border">
                                    ...
                                    <div class="input-group">
                                        <select type="text" class="form-control" name="country" id="country">
                                            ...
                                            @foreach ($data['countries'] as $item)
                                                <option {{($data['item']?->country == $item?->name) ? 'selected':''}} value="{{$item?->name}}">{{$item?->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group text-left mb-3">
                                    ...
                                    <div class="input-group">
                                        <input type="number" class="form-control" name="billing_rate" id="billing_rate" value="{{$data['item']?->billing_rate}}">
                                    </div>
                                </div>
                                <div class="form-group text-left mb-3">
                                    <label class="form-label"> <b>{{pxLang($data['lang'],'fields.payout_rate')}}</b> <em class="required">*</em> <span id="payout_rate_error"></span></label>
                                    <div class="input-group">
                                        <input type="number" class="form-control" name="payout_rate" id="payout_rate" value="{{$data['item']?->payout_rate}}">
                                    </div>
                                </div>
                                <div class="form-group text-left mb-3">
                                    <label class="form-label"> <b>{{pxLang($data['lang'],'fields.validity')}}</b> <em class="required">*</em> <span id="validity_error"></span></label>
                                    <div class="input-group">
                                        <input type="text" class="form-control dp" name="validity" id="validity" value="{{\Carbon\Carbon::parse($data['item']?->validity)->format('Y-m-d')}}">
                                    </div>
                                </div>
